...
const Reels = () => {
    ...
    // --- New States for Infinite Scroll & Limit ---
    const [reelsWatched, setReelsWatched] = useState(0);
    const [hasAlerted100, setHasAlerted100] = useState(false);
    ...
    const handleUpload = async (e) => {
        ...
        } catch (err) { console.error('Upload error:', err); setIsUploading(false); }
    };
    ...
    return (
        <div style={{ backgroundColor: theme.bg, height: '100vh', position: 'relative', overflow: 'hidden', transition: '0.3s' }}>
            ...
            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, height: '60px', background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', zIndex: 1000, borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        {profilePic && <img src={profilePic} style={{ width: '35px', height: '35px', borderRadius: '50%', objectFit: 'cover', border: `2px solid ${theme.accent}` }} alt="me" />}
                    </div>
                </div>
            </div>
            ...
            <div style={{ position: 'fixed', top: '80px', left: '20px', zIndex: 100, background: theme.sidebarBg, backdropFilter: 'blur(15px)', padding: '15px', borderRadius: '20px', color: theme.text, border: `1px solid ${theme.border}`, width: '180px', textAlign: 'center' }}>
                ...
                <div style={{ fontWeight: 'bold', fontSize: '14px', marginBottom: '5px' }}>@{username}</div>
                <div style={{ fontSize: '10px', opacity: 0.6 }}>REELS WATCHED</div>
                <div style={{ fontSize: '28px', fontWeight: 'bold', color: theme.accent }}>{reelsWatched}</div>
            ...
            <div ref={containerRef} onScroll={handleScroll} style={{ height: '100vh', overflowY: 'scroll', scrollSnapType: 'y mandatory', scrollbarWidth: 'none' }}>
                {posts.map((post, index) => (
                    <div key={`${post._id}-${index}`} style={{ height: '100vh', scrollSnapAlign: 'start', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <div onDoubleClick={() => handleLike(post, "doubleTap")} style={{ width: 'auto', height: '100%', aspectRatio: '9/16', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
                            ...
                            {showHeart === post._id && <div className="heart-animation" style={{ position: 'absolute', zIndex: 10, fontSize: '100px' }}>❤️</div>}
                    ...
                    <div style={{ width: '380px', background: theme.card, padding: '30px', borderRadius: '20px', border: `1px solid ${theme.border}` }}>
                        {uploadSuccess ? (
                            ...
                            <>
                                ...
                                <input type="file" className="form-control mb-3" onChange={handleFileChange} disabled={isUploading} />
                                <textarea className="form-control mb-4" placeholder="Caption..." value={caption} onChange={(e)=>setCaption(e.target.value)} disabled={isUploading} />
                                {isUploading && (
                                    <div style={{ marginBottom: '20px' }}>
                                        <div style={{ color: theme.accent, fontSize: '12px' }}>Upload: {uploadProgress}%</div>
                                ...
                                <button onClick={handleUpload} className="btn w-100 mb-2" style={{ background: theme.accent, color: 'white' }} disabled={isUploading || !selectedFile}>Share</button>
                                <button onClick={() => setShowUpload(false)} className="btn btn-secondary w-100">Cancel</button>
                            </>
                        )}
                    </div>
                </div>
            ...
            {showComments && (
                <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: '55vh', background: theme.card, zIndex: 5000, padding: '25px', borderTop: `1px solid ${theme.border}`, borderTopLeftRadius: '25px', borderTopRightRadius: '25px', color: theme.text }}>
                    ...
                    <form onSubmit={handleCommentSubmit} className="d-flex gap-2">
                        <input className="form-control" style={{ background: isDarkMode ? '#222' : '#eee', color: theme.text }} value={newComment} onChange={(e)=>setNewComment(e.target.value)} placeholder="Say something..." />
            ...
            <style>{`
                .heart-animation { animation: heartPop 1s ease-out forwards; }
                @keyframes heartPop {
